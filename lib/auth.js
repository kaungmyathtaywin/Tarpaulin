...
exports.authorizeAssignmentAccess = async (req, res, next) => {
    try {
        ...
        const course = await findCourseByAssignmentId(assignmentId)
...
/**
 * Middleware to authorize admin/student role for posting submissions
 */
exports.authorizeSubmissionAccess = async (req, res, next) => {
    try {
        //authorize student enrolled in the course
        const course = await findCourseByAssignmentId(req.params.assignmentId)

        const studentsEnrolled = course.students
        const studenId = new ObjectId(req.user)

        if (req.role === "admin" || (req.role === "student" && studentsEnrolled.some(id => id.equals(studenId)))) {
            next()
        } else {
            return res.status(403).send({ 
                error: "Invalid authorization to access this resource." 
